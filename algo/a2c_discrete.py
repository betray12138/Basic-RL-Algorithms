...
# Discrete适用于离散动作空间，与Continuous差异不大，主要是select_action函数中策略网络动作的处理
class A2C_Discrete(object):
    ...
    def select_action(self, state: np.ndarray, is_evaluation=False) -> (np.ndarray, torch.FloatTensor):
        ...
        action_dist = dist.Categorical(logits=a_prob)
        action = action_dist.sample()
        log_prob = action_dist.log_prob(action)
        #action.cpu().numpy()
        if is_evaluation:
            return np.argmax(a_prob.detach().cpu().numpy()), None
        return np.argmax(a_prob.detach().cpu().numpy()), log_prob
    ...
    def train(self, env: gym.Env, env_test: gym.Env, writter: SummaryWriter, max_train_steps: int, 
        ...
        while cur_step < max_train_steps:
            ...
            while not done:
                ...
                episode_reward += reward
                if self.rewardScaling:
                    reward = self.rewardScaling.get_and_update(reward[0])
